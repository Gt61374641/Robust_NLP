...
def load_data(args: argparse.Namespace) -> tuple: # (dict, dict, dict, int)
    ...
    if name == 'sst2':
        dataset = load_dataset('gpt3mix/sst2', trust_remote_code=True)
        ...
        dataset = load_dataset('SetFit/SentEval-CR', trust_remote_code=True)
        ...
        dataset = load_dataset('tweet_eval', name='sentiment', trust_remote_code=True)
        ...
        # Remove neutral (label==1), keep negative(0) and positive(2)
        ...
        valid_size = int(len(df) * train_valid_split)
        test_df = df[:test_size]
        remaining = df[test_size:]
        valid_df = remaining[:valid_size]
        train_df = remaining[valid_size:]
        ...
        dataset = load_dataset('SetFit/sst5', trust_remote_code=True)
