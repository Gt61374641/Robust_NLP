...
# Disable scaled dot product attention to support higher-order gradients in bilevel optimization
...
from task.classification.generation import get_gen_file_suffix
...
def sungen_solve(args: argparse.Namespace) -> None:
    ...
    if args.training_type == 'sungen':
        ...
        dataset_dict['train'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset,
                                                                 f'train_ZG_NL_{file_suffix}.pkl'))
        ...
        dataset_dict['train'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # Combined ZeroGen is task-specific
                                                                 f'train_ZC_NL_{file_suffix}.pkl'))
    ...
    for i in range(args.sungen_outer_epoch):
        ...
        theta_optimizer.zero_grad(set_to_none=False)
...
def sungen_train(args: argparse.Namespace):
    ...
    if args.training_type in ['unigen', 'unigen_ablation_noisy_label', 'unigen_ablation_hard_label']:
        checkpoint = torch.load(os.path.join(checkpoint_save_path, 'unigen_best_theta.pt'), weights_only=True)
    else:
        checkpoint = torch.load(os.path.join(checkpoint_save_path, f'{args.training_type}_best_theta.pt'), weights_only=True)
    ...
    if args.training_type == 'sungen':
        ...
        dataset_dict['valid'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, args.task_dataset,
                                                      f'valid_ZG_NL_{file_suffix}.pkl'))
    elif args.training_type == 'unigen':
        ...
        dataset_dict['valid'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # UniGen is not dataset-specific
                                                      f'valid_UG_SL_{file_suffix}.pkl'))
    elif args.training_type == 'unigen_ablation_noisy_label':
        dataset_dict['train'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # UniGen is not dataset-specific
                                                      f'train_UG_NL_{file_suffix}.pkl'))
        dataset_dict['valid'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # UniGen is not dataset-specific
                                                      f'valid_UG_NL_{file_suffix}.pkl'))
    elif args.training_type == 'unigen_ablation_hard_label':
        dataset_dict['train'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # UniGen is not dataset-specific
                                                      f'train_UG_HL_{file_suffix}.pkl'))
        dataset_dict['valid'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # UniGen is not dataset-specific
                                                      f'valid_UG_HL_{file_suffix}.pkl'))
    elif args.training_type == 'zerogen_combined':
        ...
        dataset_dict['valid'] = ClassificationDataset(args, os.path.join(args.preprocess_path, args.task, # Combined is not dataset-specific
                                                                 f'valid_ZC_NL_{file_suffix}.pkl'))
    ...
    final_model_save_name = f'final_model_{args.training_type}_{file_suffix}.pt'
